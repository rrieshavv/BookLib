...
const bookData = {
  ...
  isbn: "978-1234567890",
  language: "English",
  format: "Hardcover (Signed First Edition)",
  pages: 342,
  rating: 4.7,
  reviewCount: 128,
  stockStatus: "In Stock (8 copies)",
  libraryStatus: "Available for in-store reading",
  description: "In this captivating tale of literary mystery, Elena Blackwood weaves a narrative that spans centuries of book collecting and the passionate souls who dedicated their lives to preserving rare manuscripts. When a young librarian discovers a hidden collection of first editions, she uncovers secrets that could change the understanding of literary history forever.",
  genres: ["Historical Fiction", "Mystery", "Literary"],
  awards: ["National Book Critics Circle Award Finalist", "Pulitzer Prize Nominee"],
  tags: ["Signed Copy", "First Edition", "Limited Release"],
  inWishlist: false,
  reviews: [
    { 
      id: "rev-001", 
      ...
      rating: 5, 
      date: "April 28, 2025", 
      comment: "Absolutely breathtaking prose. The character development is masterful, and the plot twists kept me reading until 3 AM. Easily one of my favorite books this year!" 
    },
    { 
      id: "rev-002", 
      ...
      rating: 4, 
      date: "April 15, 2025", 
      comment: "A fascinating exploration of literary history with compelling characters. The middle chapters dragged slightly, but the ending more than made up for it." 
    }
  ]
};

export default function BookProductDetails() {
  const [quantity, setQuantity] = useState(1);
  const [tab, setTab] = useState('description');
  const [isWishlisted, setIsWishlisted] = useState(bookData.inWishlist);
  ...
  return (
    ...
    <div className="max-w-6xl mx-auto p-4 bg-white">
      ...
      <div className="text-sm text-gray-500 mb-6">
        Home / Books / {bookData.genres[0]} / {bookData.title}
      </div>
        ...
        <div className="md:col-span-2">
          ...
          <div className="flex items-center mb-8 text-sm">
            <div className="mr-6 flex items-center">
              <div className={`w-3 h-3 rounded-full mr-2 ${bookData.stockStatus.includes("In Stock") ? "bg-green-500" : "bg-yellow-500"}`}></div>
              <span>{bookData.stockStatus}</span>
            </div>
            <div className="flex items-center">
              <div className={`w-3 h-3 rounded-full mr-2 ${bookData.libraryStatus.includes("Available") ? "bg-green-500" : "bg-yellow-500"}`}></div>
          ...
          <div className="border-b border-gray-200 mb-6">
            <div className="flex space-x-6">
              <button 
                ...
                className={`py-3 px-1 font-medium ${tab === 'description' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
              >
                Description
              </button>
              <button 
                onClick={() => setTab('details')}
                className={`py-3 px-1 font-medium ${tab === 'details' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
              >
                Details
              </button>
              <button 
                onClick={() => setTab('reviews')}
                className={`py-3 px-1 font-medium ${tab === 'reviews' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
              >
          ...
          <div className="mb-12">
                  ...
                  <div className="mt-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {bookData.tags.map(tag => (
                        <span key={tag} className="bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-sm border border-blue-100">
                          {tag}
                        </span>
                      ))}
                    </div>
              ...
              <div>
                ...
                <div className="mt-8 bg-gray-50 p-4 rounded-lg border border-gray-200">
                  ...
                  <div className="text-sm text-gray-700">
                    <p className="mb-2">
                      <span className="font-medium">Order Process:</span> After placing your order online, you'll receive a confirmation email with a claim code.
                    </p>
                    <p className="mb-2">
                      <span className="font-medium">Pick-up:</span> Visit our store with your membership ID and claim code to complete your purchase.
                    </p>
                    <p>
                      <span className="font-medium">Discounts:</span> Orders of 5+ books receive a 5% discount. After every 10 successful orders, earn a 10% stackable discount!
                    </p>
                  </div>
                </div>
              </div>
            ...
            {tab === 'reviews' && (
              <div>
                ...
                <div>
                  <div className="mb-6 flex items-start border-b border-gray-200 pb-6">
                    <div className="flex-shrink-0 bg-blue-100 text-blue-600 h-10 w-10 rounded-full flex items-center justify-center font-medium mr-4">
                      {bookData.reviews[0].user.charAt(0)}
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center mb-1">
                        <span className="font-medium text-gray-800 mr-2">{bookData.reviews[0].user}</span>
                        <div className="flex items-center">
                          {renderStars(bookData.reviews[0].rating)}
                        </div>
                      </div>
                      <div className="text-sm text-gray-500 mb-2">{bookData.reviews[0].date}</div>
                      <p className="text-gray-700">{bookData.reviews[0].comment}</p>
                    </div>
                  </div>
                  
                  <div className="mb-6 flex items-start">
                    <div className="flex-shrink-0 bg-green-100 text-green-600 h-10 w-10 rounded-full flex items-center justify-center font-medium mr-4">
                      {bookData.reviews[1].user.charAt(0)}
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center mb-1">
                        <span className="font-medium text-gray-800 mr-2">{bookData.reviews[1].user}</span>
                        <div className="flex items-center">
                          {renderStars(bookData.reviews[1].rating)}
                        </div>
                      </div>
                      <div className="text-sm text-gray-500 mb-2">{bookData.reviews[1].date}</div>
                      <p className="text-gray-700">{bookData.reviews[1].comment}</p>
                    </div>
                  </div>
          ...
          <div className="border-t border-gray-200 pt-8 mb-12">
            <h3 className="font-semibold text-gray-800 text-xl mb-4">You May Also Like</h3>
              ...
              {[1, 2, 3, 4].map((item) => (
                <div key={item} className="text-center">
                  <img 
                    ...
                    className="w-full mb-2 rounded shadow-md hover:shadow-lg transition-shadow"
                  />
                  <h4 className="font-medium text-gray-800 text-sm mb-1 line-clamp-1">Book Title Example {item}</h4>
                  <p className="text-sm text-gray-500 mb-1">Author Name</p>
                  <p className="text-sm font-medium text-blue-600">$21.99</p>
                </div>
              ))}
            </div>
          </div>
